...
std::vector<int64_t> wavToLip::onnxinference(int rows, int cols, int stride, float *feats, float **result) {
    auto conf = config_A->getConfig();
    ...
    *result = new float[sp[0] * sp[1] * sp[2]];
    memcpy(*result, pdata, sizeof(float) * sp[0] * sp[1] * sp[2]);
    return sp;
}

bool wavToLip::run(int rows, int cols, int stride, float *feats) {
    auto conf = config_A->getConfig();
    ...
    w1.open(out_put_file, VideoWriter::fourcc('D', 'I', 'V', 'X'), 25, Size(1080, 1920),
            true);   //创建保存视频文件的视频流  img.size()为保存的视频图像的尺寸
    //cv::Mat frame_all_cv;
    ...
    for (size_t p_data_i = 0; p_data_i < cycles_nums_total; p_data_i++) {
        ...
        torch::Tensor                 mel_tensor     = torch::zeros({(long) out_n_512.size() - 4, 5, 512},
        ...
        //        torch::Tensor frame_3_tensor_tmp = frames_3_tensor;
        ...
        torch::Tensor frames_3_tensor_re      = torch::nn::functional::interpolate(frames_3_tensor,
                                                                                   torch::nn::functional::InterpolateFuncOptions().size(
                                                                                           std::vector<int64_t>(
                                                                                                   {512, 512})).mode(
                                                                                           torch::kBilinear));
        ...
        torch::Tensor pred_label = torch::where(
                (pred_parsing_out > 0) & (pred_parsing_out < 14) & (pred_parsing_out != 7) & (pred_parsing_out != 8) &
                (pred_parsing_out != 9), 1.0, 0.0).unsqueeze(1);
        torch::Tensor ori_label  = torch::where(
                (ori_parsing_out > 0) & (ori_parsing_out < 14) & (pred_parsing_out != 7) & (pred_parsing_out != 8) &
                (pred_parsing_out != 9), 1.0, 0.0).unsqueeze(1);
        ...
        torch::Tensor pred_label_pad_1 =
                              1 - pred_label_pad.index({Slice(), Slice(), Slice(40, 512 + 40), Slice(40, 512 + 40)});
        ...
        pred_label_pad_1 = torch::nn::functional::pad(pred_label_pad_1,
                                                      torch::nn::functional::PadFuncOptions({7, 7, 7, 7}).mode(
                                                              torch::kReflect));
        ...
        torch::Tensor past_frames = total_imgs.permute({0, 2, 3, 1}).mul_(255).clamp(0, 255).to(torch::kU8).to(
                torch::kCPU);
        cv::Mat       frame_all_cv;
        ...
        for (int n = 0; n < mel_length; n++) {
            ...
            std::memcpy((void *) img_face_cv.data, past_frames[n].data_ptr(),
                        sizeof(torch::kU8) * past_frames[n].numel());
...
vector<Mat> wavToLip::getWavToLipImgs(int rows, int cols, int stride, float *feats) {
    auto                       conf   = config_A->getConfig();
    float                      *pdata = nullptr;
    const std::vector<int64_t> sp     = onnxinference(rows, cols, stride, feats, &pdata);
    ...
    cv::FileStorage fs2(conf["PROGRAME"]["FACE_BBOX_PATH"][mSelectedTemplate].as<string>(), cv::FileStorage::READ);
    cv::Mat         face_det_boxes;
    fs2["face_det_boxes"] >> face_det_boxes;

    vector<Mat> frame_result;
    ...
    for (size_t p_data_i = 0; p_data_i < cycles_nums_total; p_data_i++) {
        ...
        // 构建face信息
        VideoCapture capture(conf["PROGRAME"]["FACE_VID_PATH"][mSelectedTemplate].as<string>());
        //VideoCapture capture("C:\\ls-dev\\ls_resourse\\c_file\\template\\test.mp4");
        //int start_frame_num = p_data_i * max_data_size;
        int          start_frame_num = p_data_i * mel_length;
        capture.set(CAP_PROP_POS_FRAMES, start_frame_num);
        ...
        for (int      n           = 0; n < mel_length; n++) {
            ...
            frame_all_cv = frames_batch[n];
            img_face_cv.copyTo(frame_all_cv(cv::Rect(x1, y1, x2 - x1, y2 - y1)));
            frame_result.push_back(std::move(frame_all_cv));
            /*string img_name = "./result/img_parsing/" + std::to_string(start_frame_num + n) + ".jpg";
            imwrite(img_name, frame_all_cv);*/
